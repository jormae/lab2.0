'use client';
import { Avatar, Box, Button, CardContent, Chip, Divider, Grid, List, ListItem, ListItemText, Skeleton, Table, TableBody, TableCell, TableHead, TablePagination, TableRow, TextField, Theme, Typography, createStyles, makeStyles } from '@mui/material';
...
import { IconCheckbox, IconPrinter, IconSquare, IconUser, IconUserPlus } from '@tabler/icons-react';
moment.locale('th')
...
const LabPage = (params:any) => {
    ...
    const username:any  = typeof window !== 'undefined' ? localStorage.getItem('username') : null
  ...
  return (
    ...
    <PageContainer title={`รายการตรวจแล็บ ${strDate}`} description="this is Sample page">
        <Box>
            ...
            <div ref={componentRef} >
                    <Grid item xs={12} lg={12} >
                        ...
                        <Grid container spacing={3} mt={5}>
                            <Grid item xs={5}>
                                <Typography color="textPrimary" fontSize={20} fontWeight={500} className={sarabunT9.className}>
                                    ที่ {org?.orgDocNo}
                                </Typography>
                            </Grid>
                            <Grid item xs={2}>
                    ...
                    <Grid item xs={12} lg={12} >
                        <Grid container spacing={3}>
                            <Grid item xs={5}></Grid>
                            <Grid item xs={6}>
                                <Typography color="textPrimary" fontSize={20} fontWeight={500} className={sarabunT9.className} sx={{textIndent:'40px'}}>
                                    {strDate}
                                </Typography>
                            </Grid>
                        </Grid>
                    ...
                    <Grid item xs={12} lg={12} >
                        <Grid item lg={12}>
                            ...
                            <Typography color="textPrimary" fontSize={20} fontWeight={500} className={sarabunT9.className} sx={{textIndent:70}}>
                                ด้วย{org?.orgName}ยังขาดอุปกรณ์และน้ำยาในการตรวจวิเคราะห์ทางห้องปฏิบัติการ ไม่สามารถทำการวิเคราะห์ได้ จึงขอส่งตัวอย่างเพื่อตรวจวิเคราะห์และอ่านผลต่อไป
                            </Typography>
                        </Grid>
                    </Grid>
                <CardContent>
                ...
                <Grid container spacing={3} sx={{ position: 'relative', clear: 'both', left: 0, bottom: 0,width: '100%', }}>
                    <Grid item lg={12} >
                        <Typography color="textPrimary" fontSize={20} fontWeight={500} className={sarabunPSK.className}>
                            ผู้ติดต่อ {userInfo?.fullname} โทรศัพท์ {userInfo?.telephone} อีเมล {userInfo?.email}
                        </Typography>
                    </Grid>
                </Grid>
            </div>
