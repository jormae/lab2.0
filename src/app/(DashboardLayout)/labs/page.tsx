'use client';
import { Alert, Box, Button, CardContent, Chip, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, FormControl, FormHelperText, Grid, InputLabel, MenuItem, Select, Skeleton, Table, TableBody, TableCell, TableHead, TablePagination, TableRow, TextField, Typography } from '@mui/material';
...
const LabPage = (params:any) => {
  ...
  return (
    ...
    <PageContainer title="LAB 2.0 : Lab Report" description="this is Sample page">
    <Toaster />
      <DashboardCard title={`รายชื่อผู้มารับบริการ รายการตรวจแล็บ ${strDate}`}  action={
          <Box mb={1}>
            ...
            <Box sx={{ overflow: 'auto', width: { xs: '280px', sm: 'auto' } }}>
                <CardContent>
                    ...
                    <form noValidate autoComplete='off'>
                        <Grid container spacing={5}>
                            <Grid item xs={12}>
                            <TextField fullWidth label='ค้นหา' placeholder='ค้นหาเลขที่บัตรประชาชน-ชื่อผู้มารับบริการ' {...register('search', {
                            ...
                            <Grid container spacing={5}>
                                <Grid item xs={12} mt={3}>
                                    <FormControl fullWidth >
                                        <InputLabel id="demo-simple-select-label">ประเภทแล็บ</InputLabel>
                                        <Select value={labTypeId ?? ''} label="ประเภทแล็บ" onChange={handleLabTypesChange} >
                                            {labTypes.blogs.map((item:any) => {
                                                return (
                                                    <MenuItem key={item.labTypeId} value={item.labTypeId}>
                                                        {item.labTypeName}
                                                    </MenuItem>
                                                )
                                            })}
                        ...
                        >
                            ...
                            <TableBody>
                                ...
                                isLoading ? (
                                    ...
                                    labs.blogs.filter((row : any)=>{
                                        return search.toLowerCase() === '' ? row : (row['patientName'].toLowerCase().includes(search) || row['idcard'].includes(search));
                                    }).slice(pg * rpg, pg *
                                    ...
                                    <TableRow key={row.visitno}>
                                        ...
                                        <TableCell>
                                            ...
                                            >
                                                <Box>
                                                    ...
                                                    >
                                                        {dayjs(row.datecheck).format('DD/MM/BBBB')} {row.timestart}
                                                    </Typography>
                                                </Box>
                                            </Box>
                                        </TableCell>
                                        <TableCell align='center'>
                                            <Typography color="textSecondary" variant="subtitle2" fontWeight={400}>
                                                ...
                                                sx={{
                                                    px: "4px",
                                                    backgroundColor: row.sex == 1 ? 'success.main' : 'warning.main',
                                                    color: "#fff",
                                                }}
                                                size="small"
                                        ...
                                        <TableCell align="center">
                                            <Button color={row.labTypeName ? "success" : "error"} variant='outlined' startIcon={<IconFlask width={18} />} onClick={e => handleClickOpen(row['visitno'], row['labTypeId'], row['sex'])} sx={{ alignContent: "right" }} value={row['labTypeId']}>
                                                {row.labTypeName ?? 'เลือกประเภทแล็บ'}
                                            </Button>
                                        </TableCell>
                                        <TableCell align="right">
                                            <Link href={`/labs/visit-no/${row['visitno']}`}>
                                                <Button color="primary" variant='contained' startIcon={<IconUser width={18} />}>
                                                    รายละเอียด
                                                </Button>
                                            </Link>
                                        </TableCell>
                ...
                <Link href={`/labs/print/${dayJs}`}>
                    <Button startIcon={<IconPrinter/>} variant='outlined' color="primary">พิมพ์ใบปะหน้า</Button>
                </Link>
            </Box>
        </DashboardCard>
    </PageContainer>
